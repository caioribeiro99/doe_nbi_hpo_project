...
Per-method execution intensity follows a pre-registered
execution-tier policy
(\texttt{benchmarks/doctoral/openml\_cc18/execution\_policy.csv} and
\texttt{execution\_tiers.md}): expensive methods (multi-fidelity,
multi-objective evolutionary, the proposed method, and ablations)
are gated through the four staged top-ups, and the final
top-up to 30 replicas requires an explicit cost sign-off based on
the observed stage-2 wall-clock. The campaign therefore degrades
gracefully to a stage-2 (10-replica) snapshot if the projected
stage-3 wall-clock proves prohibitive.
...
\begin{table}[t]
  ...
  \begin{tabular}{lllll}
    ...
    \texttt{asha}                          & multi-fidelity   & primary    & single & \citep{li2017hyperband,li2020asha} \\
