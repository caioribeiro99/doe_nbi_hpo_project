"""Tests for the Stage-3 / replica_002 standard-lane runner (Commit 48).

``scripts/run_stage3_replica002_standard_lane.py`` is the first
full-panel Stage-3 / top-up execution on top of the Commit 45
sign-off, the Commit 46 plan, and the Commit 47 single-shard pilot.
Its scope is:
...
- the runner refuses when the Commit 47 pilot summary is missing,
  failed, of the wrong scope (wrong replica / lane), or drifted on
  ``policy_version``;
- the classifier puts standard-canary rows in
  ``runnable_standard``, heavy in ``deferred_heavy_lane``, extreme
  in ``deferred_extreme_lane``, and non-canary in
  ``refused_not_in_canary_set``;
- the pre-run plan across the 10 committed shards reports
  **684** runnable standard-lane canary cells;
...
RUN_SCRIPT = REPO / "scripts/run_stage3_replica002_standard_lane.py"
...
def _write_pilot(
    ...
    record = {
        ...
        "runtime_seconds_runner_total": 341.6,
    }
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(record, indent=2), encoding="utf-8")
    return path
...
def test_run_script_help_exits_zero() -> None:
    ...
    assert "run_stage3_replica002_standard_lane.py" in out
    assert "--signoff-file" in out
    assert "--topup-plan-summary" in out
    assert "--pilot-summary" in out
...
def test_run_script_dry_run_reports_planned_not_executed() -> None:
    ...
    assert payload["expected_standard_canary_cells"] == 684
    ...
    assert payload["commit47_pilot_execution_status"] == "executed"
    assert payload["commit47_pilot_n_jobs_success"] == 68
...
def test_refuses_when_signoff_missing(tmp_path: Path) -> None:
    from scripts.run_stage3_replica002_standard_lane import (
        GateRefusalError,
        verify_signoff,
    )
...
# ---------------------------------------------------------------------------
# Commit 47 pilot summary refusal
# ---------------------------------------------------------------------------


def test_refuses_when_pilot_summary_missing(tmp_path: Path) -> None:
    from scripts.run_stage3_replica002_standard_lane import (
        GateRefusalError,
        verify_pilot_summary,
    )

    with pytest.raises(GateRefusalError, match="Commit 47 pilot"):
        verify_pilot_summary(
            tmp_path / "absent.json",
            expected_policy_version=PINNED_POLICY_VERSION,
        )
...
def test_refuses_when_pilot_summary_not_executed(tmp_path: Path) -> None:
    from scripts.run_stage3_replica002_standard_lane import (
        GateRefusalError,
        verify_pilot_summary,
    )
    ...
    p = _write_pilot(
        tmp_path / "pilot.json", execution_status="planned_not_executed",
    )
...
def test_refuses_when_pilot_summary_failed(tmp_path: Path) -> None:
    from scripts.run_stage3_replica002_standard_lane import (
        GateRefusalError,
        verify_pilot_summary,
    )
    ...
    p = _write_pilot(
        tmp_path / "pilot.json", n_jobs_failed=5, n_jobs_success=68,
    )
...
def test_refuses_when_pilot_summary_pending(tmp_path: Path) -> None:
    from scripts.run_stage3_replica002_standard_lane import (
        GateRefusalError,
        verify_pilot_summary,
    )
    ...
    p = _write_pilot(
        tmp_path / "pilot.json", n_jobs_pending_after=2,
    )
    with pytest.raises(GateRefusalError, match="n_jobs_pending_after"):
        verify_pilot_summary(
            p, expected_policy_version=PINNED_POLICY_VERSION,
        )


def test_refuses_when_pilot_summary_wrong_replica(tmp_path: Path) -> None:
    from scripts.run_stage3_replica002_standard_lane import (
        GateRefusalError,
        verify_pilot_summary,
    )

    p = _write_pilot(tmp_path / "pilot.json", replica=3)
...
def test_refuses_when_pilot_summary_wrong_lane(tmp_path: Path) -> None:
    from scripts.run_stage3_replica002_standard_lane import (
        GateRefusalError,
        verify_pilot_summary,
    )

    p = _write_pilot(tmp_path / "pilot.json", lane="heavy")
...
def test_refuses_when_pilot_summary_policy_drift(tmp_path: Path) -> None:
    from scripts.run_stage3_replica002_standard_lane import (
        GateRefusalError,
        verify_pilot_summary,
    )

    p = _write_pilot(tmp_path / "pilot.json", policy_version="f" * 64)
    with pytest.raises(GateRefusalError, match="policy_version"):
        verify_pilot_summary(
            p, expected_policy_version=PINNED_POLICY_VERSION,
        )
...
def test_refuses_when_pilot_summary_source_drift(tmp_path: Path) -> None:
    from scripts.run_stage3_replica002_standard_lane import (
        GateRefusalError,
        verify_pilot_summary,
    )

    p = _write_pilot(tmp_path / "pilot.json", source_shards_unchanged=False)
...
def test_classifier_buckets_match_policy() -> None:
    ...
    from scripts.run_stage3_replica002_standard_lane import classify_rows
...
def test_pre_run_plan_across_all_10_shards() -> None:
    """The pre-run plan over the 10 committed stage-0 shards must
    report the same totals as Commit 40's standard-lane pass — i.e.
    684 runnable standard-lane canary cells across 2,304 total."""
    from doe_xgb.runtime_guardrails import RuntimeGuardrails
    from scripts.run_stage3_replica002_standard_lane import build_pre_run_plan
...
def test_run_root_default_lives_under_runs_and_is_gitignored() -> None:
    from scripts.run_stage3_replica002_standard_lane import DEFAULT_RUN_ROOT
...
def test_stage_runs_jsonmd_are_allowlisted() -> None:
    res = subprocess.run(
        ["git", "check-ignore", "--no-index", "-v",
         ...
         "experiments/_stage_runs/"
         "stage3_replica_002_standard_lane_latest_summary.md",
         "experiments/_stage_runs/"
         "stage3_replica_002_standard_lane_latest/extras.bin"],
        cwd=REPO, capture_output=True, text=True, check=False,
    )
    assert res.returncode == 0, (res.stdout, res.stderr)
...
SUMMARY_REQUIRED_KEYS = (
    ...
    "commit47_pilot_summary_path",
    "commit47_pilot_summary_sha256",
    "commit47_pilot_n_jobs_success",
    "commit47_pilot_runtime_seconds",
    ...
    "standard_tasks_executed", "standard_tasks_in_universe",
    ...
    "only_replica_002_standard_lane_executed",
...
def test_skip_train_copies_all_10_shards_and_preserves_md5(
    ...
    summary = _run_skip_train(tmp_path, run_id="t_c48_copy_md5")
    md5_after = {
        p.name: _md5(p) for p in sorted(SHARDS_DIR.glob("shard_*.sqlite"))
    }
    ...
    # Source MD5 captured before / after copy / after rewrite / after run
    for ms in (
        summary["source_shard_md5_before"],
        summary["source_shard_md5_after_copy"],
        summary["source_shard_md5_after_rewrite"],
        summary["source_shard_md5_after"],
    ):
        assert ms == md5_before


def test_skip_train_all_10_copies_have_replica_2_and_topup_stage(
    tmp_path: Path,
) -> None:
    summary = _run_skip_train(tmp_path, run_id="t_c48_replica2")
    exec_dir = (
        tmp_path / "runs" / "t_c48_replica2"
        / "shards" / "stage0_replica_001"
    )
    ...
    assert len(summary["execution_copy_rewrite"]) == 10
    for rw in summary["execution_copy_rewrite"]:
        assert rw["target_replica"] == 2
        assert rw["target_stage"] == "stage1_topup_to_005"
...
def test_skip_train_classifies_lanes_correctly(tmp_path: Path) -> None:
    ...
    _run_skip_train(tmp_path, run_id="t_c48_lane_isolation")
    exec_dir = (
        tmp_path / "runs" / "t_c48_lane_isolation"
        / "shards" / "stage0_replica_001"
    )
    heavy_or_extreme_tids = (
        6, 167121,  # extreme
        32, 219, 3573, 7592, 9910, 9981, 14965, 146195,
        146825, 167119, 167120, 167124, 167125,  # heavy
    )
    for exec_p in sorted(exec_dir.glob("*.execution.sqlite")):
        cx = sqlite3.connect(f"file:{exec_p}?mode=ro", uri=True)
        try:
...
def test_skip_train_summary_has_all_required_keys(tmp_path: Path) -> None:
    _run_skip_train(tmp_path, run_id="t_c48_schema")
    json_p = tmp_path / "stage_runs" / "t_c48_schema_summary.json"
    ...
    assert isinstance(payload["commit47_pilot_summary_sha256"], str)
    assert len(payload["commit47_pilot_summary_sha256"]) == 64
    assert payload["execution_status"] == "executed"
    assert payload["topup_tier"] == "topup_to_5_partial"
    assert payload["lane"] == "standard"
    assert payload["expected_standard_canary_cells"] == 684
    assert payload["n_jobs_total"] == 2304
    assert payload["n_jobs_deferred_heavy"] == 156 + 267
    assert payload["n_jobs_deferred_extreme"] == 66
    assert payload["n_jobs_refused_non_canary"] == 1131
    ...
    assert payload["only_replica_002_standard_lane_executed"] is True
...
def test_skip_train_refuses_on_missing_signoff(tmp_path: Path) -> None:
    from scripts.run_stage3_replica002_standard_lane import GateRefusalError

    with pytest.raises(GateRefusalError, match="signoff file not found"):
        _run_skip_train(
            tmp_path,
            signoff=tmp_path / "absent_signoff.json",
            run_id="t_c48_no_signoff",
        )
...
def test_skip_train_refuses_on_missing_topup_plan(tmp_path: Path) -> None:
    ...
    with pytest.raises(GateRefusalError, match="stage3 top-up plan"):
        _run_skip_train(
            ...
            run_id="t_c48_no_plan",
        )


def test_skip_train_refuses_on_missing_pilot_summary(tmp_path: Path) -> None:
    from scripts.run_stage3_replica002_standard_lane import GateRefusalError

    with pytest.raises(GateRefusalError, match="Commit 47 pilot"):
        _run_skip_train(
            tmp_path,
            pilot=tmp_path / "absent_pilot.json",
            run_id="t_c48_no_pilot",
        )
...
def test_default_paths_resolve_to_committed_artifacts() -> None:
    ...
    assert DEFAULT_PILOT_SUMMARY == PILOT_SUMMARY
    assert TARGET_REPLICA == 2
    assert TARGET_STAGE_LABEL == "stage1_topup_to_005"
    assert EXPECTED_STANDARD_CANARY_CELLS == 684
    assert PV == PINNED_POLICY_VERSION


def test_live_signoff_plan_and_pilot_match_pinned_policy() -> None:
    """The committed signoff, Stage-3 top-up plan, and Commit 47 pilot
    must all pin the same policy_version as the live
    ...
    assert pilot["policy_version"] == live_pv
    assert plan["execution_status"] == "planned_not_executed"
    assert pilot["execution_status"] == "executed"
    assert int(pilot["replica"]) == 2
    assert pilot["lane"] == "standard"
    assert int(pilot["n_jobs_success"]) == 68
    assert int(pilot["n_jobs_failed"]) == 0
    assert int(pilot["n_jobs_pending_after"]) == 0
    tier_topup_5 = next(
