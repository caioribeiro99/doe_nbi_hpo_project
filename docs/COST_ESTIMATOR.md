...
> **Note (Commit 26, refined in Commit 27).** The estimates below
> cover one method per (task, algorithm) cell. The frozen CC18
> protocol runs **12 full-CC18 methods** plus **1 subset-only
> method** (ParEGO on 48 of 72 tasks); see
> `benchmarks/doctoral/openml_cc18/job_count_projection.md` for the
> per-stage job count and the dedicated-Mac wall-clock projection
> under the execution-tier policy. At full intensity, the
> 30-replica campaign is ~207 dedicated-Mac days at efficiency 0.85
> — the policy gates stage 3 (top-up to 30 replicas) behind a
> manual sign-off for every tier 1+ method, so the campaign can ship
> a stage-2 (10-replica) snapshot at ~65 days if the stage-3 cost
