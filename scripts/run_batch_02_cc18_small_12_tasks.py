#!/usr/bin/env python
"""Run batch_02_cc18_small_12_tasks against real OpenML data via the
result handoff protocol (Commit 35).

This is the second real-OpenML batch in the doctoral pipeline. It is
the first one that uses the protocol formalized in Commit 35:

- it materializes a gitignored execution copy of every committed
  stage-0 shard that contributes batch_02 rows under
  ``runs/cc18/<run_id>/shards/stage0_replica_001/`` via
  ``scripts/create_cc18_run_dir.py``;
- it runs the canary methods × 3 GBDT algorithms × 12 tasks against
  those execution copies (committed source shards never opened for
  write);
- it caches OpenML payloads under ``data/source/openml_cc18/<task>/``
  (already gitignored since Commit 34);
- it publishes a stage-run summary at
  ``experiments/_stage_runs/batch_02_cc18_small_12_tasks_latest_summary.{json,md}``
  via ``scripts/export_cc18_run_summary.py``.

Pre-flight refusals
-------------------
- ``experiments/_batch_runs/batch_01_cc18_tiny_3_tasks_latest.json``
  must exist, be green (36/36 success), report
  ``source_shards_unchanged: true``, and be ≤ 7 days old;
- ``stage3_signoff.json`` must NOT exist;
- the batch CSV must contain exactly the 12 task IDs documented in
  ``benchmarks/doctoral/openml_cc18/batches/batch_02_cc18_small_12_tasks.meta.json``.

What the runner does NOT do
---------------------------
- mutate any committed SQLite shard;
- run anything outside the canary set;
- run on tasks outside the 12 in the batch CSV;
...
DEFAULT_BATCH_META = (
    REPO / "benchmarks/doctoral/openml_cc18/batches/batch_02_cc18_small_12_tasks.meta.json"
)
DEFAULT_BATCH01_GATE = (
    REPO / "experiments/_batch_runs/batch_01_cc18_tiny_3_tasks_latest.json"
)
DEFAULT_SHARDS_DIR = REPO / "jobs/doctoral/openml_cc18/shards/stage0_replica_001"
DEFAULT_RUN_ROOT = REPO / "runs/cc18"
DEFAULT_OUT_ROOT = REPO / "experiments/_batch_runs/batch_02_cc18_small_12_tasks"
...
class GateRefusalError(RuntimeError):
    """Raised when the batch_01 pre-flight checks reject the run."""


def _gate_age_days(gate_path: Path) -> float:
    ...
    try:
        ...
        raise GateRefusalError(
            f"batch_01 gate has unparsable run_timestamp={ts!r}: {exc}"
        ) from exc
    delta = datetime.now(timezone.utc) - run_dt
    return delta.total_seconds() / 86400.0


def verify_batch01_gate(
    gate_path: Path = DEFAULT_BATCH01_GATE,
    *, max_age_days: float = BATCH_01_MAX_AGE_DAYS,
) -> dict:
    """Refuse batch_02 if the batch_01 gate is missing, failed, or stale."""
    if not gate_path.exists():
        raise GateRefusalError(
            f"batch_01 latest artifact not found at {gate_path}; "
            "run scripts/run_batch_01_cc18_tiny_3_tasks.py first."
        )
    payload = json.loads(gate_path.read_text(encoding="utf-8"))
    ...
    if expected != 36 or success != 36 or failed != 0:
        raise GateRefusalError(
            f"batch_01 gate is not green: expected={expected} "
            f"success={success} failed={failed}"
        )
    if not unchanged:
        raise GateRefusalError(
            "batch_01 gate reports source_shards_unchanged=False; "
            "investigate before running batch_02."
        )
    if signoff:
        raise GateRefusalError(
            "batch_01 gate reports stage3_signoff_present=True; "
            "refusing to run batch_02 in pre-stage-0 territory."
        )
    age = _gate_age_days(gate_path)
    if age > float(max_age_days):
        raise GateRefusalError(
            f"batch_01 gate is {age:.2f} days old (>{max_age_days:.0f}d); "
            "re-run batch_01 before batch_02."
        )
    return {
...
def run_batch_02(
    *,
    ...
    batch01_gate: Path = DEFAULT_BATCH01_GATE,
    max_age_days: float = BATCH_01_MAX_AGE_DAYS,
    ...
    """Run the batch_02 canary on real OpenML data and return the
    ...
    batch01_summary = verify_batch01_gate(
        batch01_gate, max_age_days=max_age_days,
    )

    if SIGNOFF_FILE.exists():
        raise GateRefusalError(
            f"refusing to run batch_02: stage-3 sign-off file already exists "
            f"at {SIGNOFF_FILE}"
        )
    ...
    if len(task_ids) != 12:
        raise GateRefusalError(
            f"{batch_csv}: expected 12 task IDs, got {len(task_ids)}"
        )
    ...
    #    matching canary row for the 12 task IDs).
    ...
    # 3. Prune each execution copy down to the batch_02 canary slice.
    n_pruned_total = 0
    ...
    #    but we keep an explicit dict so the augmented batch_02 summary
    ...
    # 9. Augment the summary with batch_02-specific blocks (task
    ...
    # level verdict from export_summary, but we append a batch_02-
...
def _augment_md(md_path: Path, summary: dict) -> None:
    """Append the batch_02 task / per-cell blocks to the summary MD."""
    extra: list[str] = []
    extra.append("\n---\n")
    extra.append("## batch_02-specific summary\n")
    extra.append(
    ...
    g = summary["batch_01_gate"]
    extra.append("### batch_01 pre-flight\n")
    extra.append(f"- run_timestamp: `{g.get('run_timestamp')}`")
    ...
    if (
        ...
        extra.append("### batch_02 verdict: **GATE PASS**\n")
        extra.append(
            "batch_03_cc18_representative_18_tasks may proceed (only "
            "after manual review and only via the same handoff "
            "protocol).\n"
        )
    else:
        extra.append("### batch_02 verdict: **GATE FAIL**\n")
        extra.append(
            "Resolve failures, restore committed shards, or remove "
            "the stage-3 sign-off file before attempting batch_03.\n"
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--batch01-gate", type=Path, default=DEFAULT_BATCH01_GATE,
    )
    parser.add_argument(
        "--max-age-days", type=float, default=BATCH_01_MAX_AGE_DAYS,
    )
    ...
    if args.dry_run:
        print(json.dumps({
            ...
            "batch01_gate": str(args.batch01_gate),
    ...
    try:
        summary = run_batch_02(
            ...
            batch01_gate=args.batch01_gate,
            max_age_days=args.max_age_days,
            max_evaluations=args.max_evaluations,
            n_folds=args.n_folds,
            run_id=args.run_id,
