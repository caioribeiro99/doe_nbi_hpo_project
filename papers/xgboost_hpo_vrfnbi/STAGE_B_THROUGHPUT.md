...
> **CORRECTED after the final protocol review.** The figures first published here understated the
> campaign by 39%. They counted four arms where five run (HISTORICAL-WS runs twice, bit-faithfully
> and under the shared specification), omitted the anchor-injection control, and charged the payoff
> matrix two evaluations that are now reused from the anchor search. The corrected accounting is
> below and is regenerated by `scripts/xgb_hpo_campaign.py plan`.
...
| evaluations per replication per dataset | **3,168** | 2,376 |
| campaign evaluations (R = 30, 4 datasets) | **380,160** | 285,120 |
| unmatched NSGA-II, one replication per dataset | **15,360** | not counted |
| **campaign total** | **395,520** | 285,120 |
| **projected wall clock** | **17.4 h, 0.73 days** | 12.6 h, 0.52 days |

The correction does **not** disturb the objective-count decision. That was refused on scientific
grounds, set out in `protocol/Q3_AMENDMENT_REVIEW.md`: the candidate objective failed the surrogate
reliability gate on two of the four datasets, and what the second quality axis measures differs from
one dataset to the next. Compute never entered it, and 0.73 days remains an order of magnitude inside
the five-day ceiling and well inside the four-day threshold fixed before any throughput number was
read.
