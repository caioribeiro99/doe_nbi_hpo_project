...
| **Diagnosis, CORRECTED** | the guard was a proxy for inversion and the proxy is wrong. Measured directly, the composite is **not inverted anywhere** on any dataset. **The mechanism behind the divergence is structural, not a property of any one response.** The quality composite is a weighted sum of rotated quality factors and the cost objective is another factor from the same orthogonal basis, so `Pearson(quality, cost)` is **zero by construction on the sample the model is fitted to**, and approximate but **not bounded by any small constant** away from it. **Two earlier versions of this row were wrong**: the first claimed the identity held everywhere for any dataset; the second claimed |r| ≤ 0.04 on any subsample, which is false. A third stated measured percentiles inline, and those were silently falsified when amendment 23 refitted the models — which is why **every figure has been removed from this row**. A Spearman between two variables whose linear association is structurally zero is a monotone-nonlinearity residual. **It is not noise** — it is stable within a dataset. What it lacks is a consistent **direction** across the panel: negative on one dataset, positive on two, and indeterminate on the fourth. Comparing that sign against the sign of a genuine raw-response conflict was never an invariant. All measurements live in `audits/latent_conflict_stability.json` and are deliberately not duplicated here. |
...
| | |
|---|---|
| **The measurement** | Spambase's non-dominated set of design rows contains **two points**, the two extreme anchors, so its curvature is undefined and screening criterion 2 is **not met**. MAGIC, Adult and Bank Marketing meet it with 7, 6 and 6 front points and curvature 0.173, 0.199, 0.198. `audits/final_panel_screening.json`. (An earlier version of this row carried 8, 6, 6 and 0.166, 0.211, 0.214 — the figures as measured before amendment 23 refitted the models. The role assignment is unchanged.) |
...
| **What changed numerically** | every frozen model was refitted and every dependent artifact regenerated. The **panel roles are unchanged**, and the numbers behind them are not. MAGIC's non-dominated set lost a point, from 8 to **7**, and its curvature moved from 0.166 to **0.173**; Adult and Bank Marketing stayed at 6 points with curvature 0.199 and 0.198; Spambase still fails with **two**. An earlier version of this row asserted "8, 6, 6 ... 0.166" — the pre-refit figures — while claiming the refit changed nothing, which is the error this amendment exists to correct in the opposite direction. The substantive claim survives and is stronger stated honestly: refitting on a different sample moved the geometry measurements, and the role assignment was **robust to that movement** rather than unaffected by it. Criterion 1 is unchanged to three decimals. Composite alignment stays positive everywhere: +0.929, +0.243, +0.835, +0.896. The detectable-effect range moves from 0.8–2.8 to 0.8–2.8 percentage points uncorrected and 2.3–8.0 corrected. |
