...
entirely, once anchors are correct, adds much less on three of the four datasets. The fix is also cheap: on the
convex log-loss the real anchors are one SLSQP call, so NBI-B costs essentially what NBI-A costs while repairing most
of the gap to the metamodel-free arm.
...
and the front collapses (\cref{sec:results:anchors}) --- a geometric failure, not a numerical one, since the solver
certifies these subproblems, and invisible to any diagnostic computed on the surface alone.
...
machinery works as advertised: Santander's log-loss surface passes the gate in every partition.
...
$4.2\times10^{5}$ real evaluations per replication against none for the surrogate arms, at 3 to 73 times the
...
the simplex, and the consequence is not cosmetic: the hypervolume-best method changes between the definitions in up to
$24$ of 30 partitions (\cref{sec:results:cost}), and several paired conclusions reverse, including the anchor effect
on UCI credit.
...
objective that is a step function of the support.

What we would carry forward is not a method but a checklist: external validation on unseen compositions, an argmax
check against the real optimum, revalidation on the true objective before scoring, a reference whose sampled core no
method under test can influence, and an explicit statement of which cost is paid. Each of them changed a conclusion
