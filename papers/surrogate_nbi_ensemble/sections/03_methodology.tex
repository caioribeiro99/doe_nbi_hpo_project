...
stages it depends on rather than its solver stage alone (itemized in \cref{tab:compute}). Real anchors are not free:
NBI-B consumes about $4\times10^{4}$ real evaluations that NBI-A does not --- the direct ROC-AUC search of
\cref{sec:method:refs}, roughly a tenth of NBI-C's entire budget --- and its standalone wall clock is $1.2$--$12.7$
times NBI-A's.
...
$\mathcal{F}$. Adding the NSGA-II arm moves every indicator level slightly: on identical candidate sets NBI-C's median
hypervolume ratio is $0.989$, $0.983$, $0.982$ and $0.976$ without NSGA-II in the reference
(\cref{sec:results:metamodelfree}) and $0.990$, $0.981$, $0.981$ and $0.977$ with it (\cref{tab:nsga2}). Absolute
indicator levels are therefore only interpretable together with the set of methods that built the reference.
