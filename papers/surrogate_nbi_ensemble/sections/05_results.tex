...
points is approximately zero everywhere, so the surrogate is not predicting outside its fitted region. As a control,
the Brier score --- genuinely quadratic in $\wvec$ --- is reproduced with $\rext = 1.000$ in all 120 replications, so
design and fitting are sound and the failure is specific to the response (\cref{fig:surrogate}).
...
per-partition largest in seven of them, and between-partition coefficients of variation for the leading terms lie
between $0.011$ and $0.17$ (per-cell detail in supplementary Section~S3).
...
this, and optimal rather than 50/50 weighting does not rescue it (supplementary Section~S3). Across all ten pairs of
every dataset (\cref{tab:edge}) all 20 disagreements between criterion and reality are the surface predicting a blend
the real objectives do not deliver, and none is a real one missed.
...
\texttt{nsga2/\allowbreak nsga2\_\allowbreak pareto\_\allowbreak quality.csv}, moves the magnitudes without changing any direction here and leaves the
ordering above inverted in the same way (supplementary Section~S5).
...
$+0.279$ is over four times the median $+0.058$ --- so by the rule fixed in advance we report the median with its
bootstrap interval $[+0.051, +0.070]$, the win fraction with its Wilson interval and the rank-biserial correlation
...
batch difference is itself significant under an identical protocol, supplementary Section~S11): the effect is real in
...
worse in $26$--$27/30$ partitions (full support-cost repetition in supplementary Section~S5).
...
A-to-B hypervolume gap on Santander, $90\%$ on BNP Paribas, $49\%$ on Porto Seguro and $1\%$ on UCI credit. The
residual, which is the part attributable to the relocated geometry, is $+0.038$, $+0.005$, $+0.068$ and $+0.097$
...
more volume ($24/30$, median $+0.0106$) while NBI-B is closer in $\igdp$ ($24/30$); against the method-independent
sample core the $\igdp$ comparison reverses --- NBI-C is closer in $20/30$ --- while the hypervolume direction is
...
Third, the premium is large and uneven (\cref{tab:compute}): NBI-C consumes about $4.2\times10^{5}$ real objective
evaluations per replication where the surrogate arms consume none. Charged standalone, so that each arm carries the
stages it depends on, the premium of NBI-C over NBI-B is $6.2$, $5.9$, $5.9$ and $2.3\times$ --- smallest on UCI
credit, where it buys the largest gain, and 73-fold on Santander for $+0.008$ hypervolume on a dataset whose log-loss
surface is already exact. Metamodel-free NBI is the safe default, justified where the surfaces or the anchors fail,
not uniformly preferable.
...
NBI-C on UCI credit (differences within $\pm 0.002$, win fractions $16/30$ and $18/30$, corrected $p = 1.00$), matches
it on Santander hypervolume ($0.978$ against $0.989$) and beats it on Santander $\igdp$ in $20/30$ --- and collapses
where they are not, to a median of one front point and hypervolume ratio $0.474$ on BNP Paribas. The comparison is
...
The 66-point random Dirichlet sample loses to every method everywhere --- its hypervolume ratio is exactly zero in up
to $25/30$ partitions (supplementary Section~S4), because uniform compositions cost 8 to 20 times more than the
reference front, and the unoptimized 66-run mixture design also beats it $30/30$ everywhere --- but it is matched to
the NBI sets only in the number of candidates returned, so this is a floor check and no efficiency claim is made from
it.
...
repetition is in supplementary Section~S13). All 16 dataset $\times$ endpoint $\times$ reference cells favour NSGA-II, with
...
delivered is a change of status for three findings --- the previously undecidable Porto Seguro and UCI credit anchor
effect, the BNP Paribas anchor collapse, now a quantified $7/30$ with an identified trigger, and two cost artifacts
exposed as such (supplementary Section~S11). A fourth item on that list has since been withdrawn: the NBI-C versus
NBI-B hypervolume/$\igdp$ split on BNP Paribas is a property of the reference definition and reverses under the sample
core (\cref{sec:results:metamodelfree}), so the extension sharpened that estimate without settling a finding. The
criterion is itself lenient, because partitions 0--9 are one third of the $R = 30$ sample: against the twenty new
partitions alone only 6 of 12 hypervolume and 5 of 12 $\igdp$ cells agree, and the Porto Seguro and UCI credit anchor
win rates differ between batches under an identical protocol. That disagreement is the magnitude of partition
sensitivity the replication was designed to measure (supplementary Section~S11).