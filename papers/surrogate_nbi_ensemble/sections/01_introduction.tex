...
\citep{das1998nbi}, so errors there propagate globally; the classical ideal/nadir literature
\citep{messac2003normalized,deb2010nadir} asks about estimating the true objectives, not about a surrogate standing in
for them.
...
\item \textbf{NBI-C} drops the metamodel and runs the same NBI on the exact out-of-fold objectives; B versus C
isolates residual \emph{interior surrogate error}.
\end{itemize}
...
Four public binary tabular datasets are partitioned 30 times each, giving 120 replications with recorded seeds.
...
misplacement and solver behaviour}, through three NBI variants defined by two methodological factors. No prior study
we found computes NBI anchors from both a surrogate and the true objectives and compares the fronts; the nearest
precedent \citep{kwon2024ensemble} optimizes one metric at a time and has no anchors or front.
...
often enough to change the winning method in up to 24 of 30 partitions.
\end{enumerate}
