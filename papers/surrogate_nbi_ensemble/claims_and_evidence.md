...
- "HV ≥ 0.97 on every dataset": NBI-C weighted HV ratio ≥ 0.97 in 30/30 partitions of each dataset (FINAL_REPORT §11-A; per-partition rows in `tables/pareto_quality.csv`, set = nbi_C, cost = weighted).
...
- Compute premium (`tables/stage_times.csv`, mean s per partition A / B / C): Santander 34 / 35 / 2,528; BNP 23 / 36 / 1,410; Porto 85 / 56 / 2,505; UCI 125 / 100 / 307; C/B = 73×, 40×, 45×, 3× (manuscript Table 5 computes from the CSV; FINAL_REPORT quotes 77×, 40×, 46×, 3× from medians/rounded means — use the Table 5 values in the text).
...
## C6 — Scheffé coefficients are partition-stable; large β_ij mark the weakest vertex, not exploitable synergy — SECONDARY (stability) / SECONDARY (blend test)
