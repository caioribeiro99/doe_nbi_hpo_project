...
that the optimizers actually minimized; the support-cost repetition is a pre-specified sensitivity analysis. The
untouched 20\% holdout enters no primary endpoint: it is used only for the confirmation analysis of
\cref{sec:results:holdout}. All other indicators are descriptive.
...
training part, whose rows two replications share to the extent computed above. The value $\rho = 0.25$ is therefore a
lower bound on the dependence between replications and the corrected test is, if anything, still anticonservative;
every significance statement below should be read as holding \emph{at} $\rho = 0.25$, with the effect consistency
that does not depend on $\rho$ reported alongside it. Within each
